...
function parseBlockNumber(value?: string): number {
  if (!value) {
    throw new Error("Missing block number in RPC response");
  }
...
function parseTimestamp(value?: string): number {
  if (!value) {
    throw new Error("Missing block timestamp in RPC response");
  }
  return Number(BigInt(value));
}
...
function isBaseUniswapV4Pair(pair: DexPair): boolean {
  return (
    pair.chainId === "base" &&
...
function pairContainsInput(pair: DexPair, tokenOut: string, tokenIn: string, nativeIn: boolean): boolean {
  ...
  const containsWrappedNative = nativeIn && tokens.some((value) => isAddressEqual(value, WRAPPED_NATIVE.base));
...
async function findBlockByTimestamp(targetTimestamp: number, latestBlock: { number: number; timestamp: number }): Promise<number> {
  ...
  // Initial estimate using average block time (~2s on Base)
  ...
  lo = Math.max(0, estimate - 500_000);
  hi = Math.min(latestBlock.number, estimate + 500_000);
...
async function resolvePoolKey(poolId: string, pairCreatedAt?: number): Promise<PoolKey> {
  ...
  // Fallback: scan recent 50K blocks
  const fallback = await scanForInitializeLog(poolId, Math.max(0, latestBlock.number - 50_000), latestBlock.number);
...
async function resolveDexScreenerPool(tokenIn: string, tokenOut: string, nativeIn: boolean): Promise<ResolvedV4Pool> {
  ...
  const lookupToken = isAddressEqual(tokenOut, WRAPPED_NATIVE.base) ? tokenIn : tokenOut;
  const pairs = (await getTokenPairs("base", lookupToken))
    .filter((pair) => isBaseUniswapV4Pair(pair) && pairContainsInput(pair, tokenOut, tokenIn, nativeIn))
    .sort(byLiquidityDesc);

  const pair = pairs[0];
  if (!pair) {
    throw new Error(`DexScreener did not return a direct Base Uniswap V4 pool for ${lookupToken}`);
  }
...
export async function buildSwapTx(params: SwapParams, fee = 3000): Promise<UnsignedSwapTx> {
  ...
  const actualTokenIn = nativeIn ? WRAPPED_NATIVE.base : tokenIn;
  const actualTokenOut = isNativeIn(tokenOut) ? WRAPPED_NATIVE.base : tokenOut;
  ...
  const wrapNativeToWeth = nativeIn && isAddressEqual(resolvedPool.inputCurrency, WRAPPED_NATIVE.base);
...
export async function getQuote(
  ...
  const nativeIn = isNativeIn(tokenIn);
  const actualIn = nativeIn ? WRAPPED_NATIVE.base : tokenIn;
  const actualOut = isNativeIn(tokenOut) ? WRAPPED_NATIVE.base : tokenOut;
